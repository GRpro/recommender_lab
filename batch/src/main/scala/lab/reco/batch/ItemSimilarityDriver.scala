...
import org.apache.mahout.math.indexeddataset.{BiDictionary, IndexedDataset, Schema, indexedDatasetDFSReadElements}
...
object ItemSimilarityDriver extends MahoutSparkDriver {
  ...
  private var indicators: Array[String] = _

  private var writeSchema: Schema = _
  private var readSchema1: Schema = _
  private var secondaryIndicatorSchemas: Seq[Schema] = _
  ...
  override def main(args: Array[String]): Unit = {
    ...
    parser = new CustomMahoutOptionParser(programName = "spark-itemsimilarity") {
    ...
    parser.parse(args, parser.opts) foreach { opts =>
      parser.opts = opts
      process()
    }
  }
  ...
  override protected def start() : Unit = {
    ...
    val indicatorList = parser.opts("indicatorList").asInstanceOf[String]
    indicators = indicatorList.split(",")

    val primaryIndicator = indicators.headOption.orNull
    val secondaryIndicators = if (indicators.nonEmpty) indicators.tail else Array.empty

    val schema = new Schema("delim" -> parser.opts("inDelim").asInstanceOf[String],
      "filter" -> primaryIndicator,
    ...
    if (secondaryIndicators.nonEmpty) {
      secondaryIndicatorSchemas = secondaryIndicators.map { indicator =>
        new Schema(readSchema1) += ("filter" -> indicator)
      }
    }
  ...
  private def readIndexedDatasets: Array[IndexedDataset] = {
    ...
    val inFiles = HDFSPathSearch(
      parser.opts("input").asInstanceOf[String],
      parser.opts("filenamePattern").asInstanceOf[String],
      parser.opts("recursive").asInstanceOf[Boolean]
    ).uris
    ...
    if (inFiles.isEmpty) {
      ...
      val secondaryIndicatorDatasets = {

        var existingRowIDs: BiDictionary = datasetA.rowIDs
        var datasets: ArrayBuffer[IndexedDataset] = ArrayBuffer.empty

        for (schema <- secondaryIndicatorSchemas) {
          val dataset = indexedDatasetDFSReadElements(inFiles, schema, existingRowIDs = Some(existingRowIDs))
          datasets += dataset
          existingRowIDs = dataset.rowIDs
        }

        datasets.toArray
      }

      if (secondaryIndicatorDatasets.nonEmpty) { // do AtB calc
        // true row cardinality is the size of the row id index, which was calculated from all rows of A and B
        val rowCardinality = secondaryIndicatorDatasets.last.rowIDs.size // the authoritative row cardinality
        ...
        val returnedSecondaryDatasets = secondaryIndicatorDatasets.map { dataset =>
          if (rowCardinality != dataset.matrix.nrow) dataset.newRowCardinality(rowCardinality)
          else dataset // this guarantees matching cardinality
        }

        Array(returnedA) ++ returnedSecondaryDatasets
      } else {
        Array(datasetA)
      }
    }
  ...
  override def process(): Unit = {
    ...
    (idss zip indicators).foreach { case (matrix, indicatorName) =>
      matrix.dfsWrite(parser.opts("output").asInstanceOf[String] + s"similarity-matrix-$indicatorName", schema = writeSchema)
    }
