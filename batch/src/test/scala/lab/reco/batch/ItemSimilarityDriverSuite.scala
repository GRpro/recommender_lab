...
class ItemSimilarityDriverSuite extends FunSuite with DistributedSparkSuite {
  ...
  */
  ...
  val CartCrossSimilarityTokens = tokenize(Iterable(
    "ipad\tmeizu:4.498681156950466 ipad:1.7260924347106847 iphone:1.7260924347106847",
    "surface\tsurface:4.498681156950466 macbook:0.6795961471815897",
    "iphone\tipad:5.545177444479561 iphone:5.545177444479561 meizu:1.7260924347106847 macbook:1.7260924347106847",
    "nexus\tgalaxy:4.498681156950466 nexus:4.498681156950466 macbook:0.6795961471815897",
    "galaxy\tgalaxy:1.7260924347106847 nexus:1.7260924347106847 macbook:1.7260924347106847"))


  test("ItemSimilarityDriver, non-full-spec CS1V") {
...
//      "u4,view,galaxy",
    ...
    val res = mahoutCtx.parallelize(lines).saveAsTextFile(InFile)
    ...
    val similarityLines = mahoutCtx.textFile(OutPath + "/similarity-matrix-purchase/").collect.toIterable
    tokenize(similarityLines) should contain theSameElementsAs SelfSimilairtyTokens
//    val viewCrossSimilarityLines = mahoutCtx.textFile(OutPath + "/similarity-matrix-view/").collect.toIterable
//    tokenize(viewCrossSimilarityLines) should contain theSameElementsAs CrossSimilarityTokens
    val cartCrossSimilarityLines = mahoutCtx.textFile(OutPath + "/similarity-matrix-cart/").collect.toIterable
    tokenize(cartCrossSimilarityLines) should contain theSameElementsAs CartCrossSimilarityTokens
  }
  ...
  test("ItemSimilarityDriver, non-full-spec CSV") {
    ...
    ItemSimilarityDriver.main(Array(
      ...
      "--output", OutPath,
      "--master", masterUrl,
      "--indicatorList", "purchase,view,cart",
      "--inDelim", ",",
      "--itemIDColumn", "2",
      "--rowIDColumn", "0",
      "--filterColumn", "1",
  ...
  test("ItemSimilarityDriver TSV ") {
    ...
    val similarityLines = mahoutCtx.textFile(OutPath + "/similarity-matrix-purchase/").collect.toIterable
    tokenize(similarityLines) should contain theSameElementsAs SelfSimilairtyTokens
    val crossSimilarityLines = mahoutCtx.textFile(OutPath + "/similarity-matrix-view/").collect.toIterable
